...
```{r message=FALSE}
read_table(example_url("diet.dat")) %>% head(n=10)
```
...
```{r}
df <- read.table(example_url("cotton.dat"), header=T, comment.char="#", sep="\t") %>% 
  tibble() %>% 
  mutate(
...
```{r}
df <- read.table(example_url("battery.dat"), comment="#", header=T) %>% 
  mutate(
...
df %>%
  ...
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin=min, ymax=max), width=0.1, position = position_jitter(width=0.1))
```

Si noti l'uso di `position=position_jitter()`, che aggiunge un piccolo offset casuale alla posizione orizzontale delle barre d'errore, in modo che non essendo sovrapposte risultino più leggibili.
...
1:3 %>% walk(
  ~ print(res[[.]]$Material %>% 
    ...
    coord_flip() +
    labs(
      title=glue("T = {levels(df$Temperature)[.]}°"),
      x="Difference", y="Pair"))
)
```
