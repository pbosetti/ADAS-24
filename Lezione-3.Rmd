...
df <- read_csv(examples_url("kfold.csv"))
df %>% 
  ggplot(aes(x=x, y=y)) +
  geom_point()
```
...
models <- df %>% 
  filter(train) %>% {
    map(1:8, \(deg) lm(y~poly(x, deg, raw=T), data=.))
  }
```
...
Osserviamo che RMSE è minimo per una regressione polinomiale di terzo grado, quindi per evitare sotto- e sovra-adattamento scegliamo un polinomio di terzo grado.
...
```{r}
data <- read_table(examples_url("soap_bottles.dat"), comment="#") %>%
  mutate(fail = !OK)
...
data <- data %>% 
  mutate(
    training = sample(c("train","valid"), n(), prob=c(80, 20), replace=T)
  ) %>% 
  select(-OK)
```
...
```{r}
data.glm <- glm(fail ~ p, family="binomial", data=filter(data, training == "train"))
summary(data.glm)
```
...
data %>% 
  ...
  labs(x="Livello di riempimento", y="predizione", color="pred. corretta")
```
...
data %>% 
  mutate(
    type = map2_chr(fail, pred>0.5, ~error_type(.x, .y))) %>% 
...
```{r}
pred <- prediction(
  filter(data, training == "train")$pred, 
  filter(data, training == "train")$fail
)
