...
> In effetti, più che un pattern osserviamo una marcata differenza dei residui in corrispondenza del valore minimo predetto (attorno a 380 $\m$m), che corrisponde poi ad una maggior distribuzione dei residui quando il fattore $ (Temperatura) è al livello -1. Più che ad un modello non adeguato, quindi, concludiamo che quando la temperatura è bassa il processo ha una maggior variabilità (e una minor resa). Dato che $A$ è il fattore più influente, le cose sono collegate. Quindi adottiamo come corretto il modello `t~A*B+A*C`.
...
dm %>% 
  ...
  add_predictions(lm(1/sqrt(Y)~A*C+A*D, data=.)) %>%
  bind_rows(
    dmf %>% mutate(model="half") %>% add_predictions(dmf.lm)
  ) %>% 
