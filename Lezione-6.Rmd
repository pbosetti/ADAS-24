...
In raltà, osservando i residui in funzione del valore predetto si può dedurre che c'è un possibile pattern (largo-stretto-largo, a U rovesciata), quindi applichiamo il metodo di Box-Cox per identificare la miglior trasformazione sulla resa:
...
In ogni caso, osserviamo che gli unici effetti significatvi sono `A`, `B`, `A:B`, `A:C`, `C`. Possiamo riformulare il modello e verificare con ANOVA:
...
**Nota**: in caso di dubbio, è meglio essere conservativi nell'analisi del grafico Q-Q e includere anche termini dubbi (ad esempio `B:D`), e poi verificarne con l'ANOVA leffettiva significatività.
...
Possiamo finalmente produrre la superficie di risposta, usando il solito metodo. Rispetto al primo esempio, siccome abbiamo un fattre in più, calcoliamo le superfici di risposta per i due fattori più importanti `A` e `B`, e sfaccettiamo i grafici per 6 livelli di `C`.
...
La trasformazione con meno interazioni di alto livello fuori dalla diaginale è l'inversa:
...
Come sempre realizziamo prima un modello completo da veridicare col metodo di Daniel:
...
Rifomuliamo il modello con solo i termini `A`, `C`, `D` e le loro interazioni:
...
Ci restano da realizzare i grafici di interazione (o la superficie di risposta, lasciata per esercizio). Per decidere come organizzare questi grafici è utile analizzare quali fattori hanno l'effetto maggiore.
...
Quindi realizziamo i grafici di interazione tra `A` e `C`, sfaccettati per `D`:
