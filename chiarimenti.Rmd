...
> * Intervalli di confidenza --> come crearli corretamente con ggplot
...
starwars %>% 
  ...
  summarise(height=median(height), number=n(), .groups="keep") %>% 
  filter(number > 1)
```
...
pop_wide %>% 
  ...
  slice_head(n=20) %>% 
  # Grafico a barre
  ggplot(aes(x=factor(country, levels=country[order(change)]), y=change)) +
  geom_col() +
...
starwars %>% 
  mutate(
    body = paste(height, "cm x", mass, "kg")
  )
```
...
Vediamo un caso in cui è veramente utile:
...
m <- mean(v)
s <- sd(v)
(pred_i <- qnorm(c(alpha/2, 1-alpha/2), lower.tail = T))
```
