...
df <- fp_design_matrix(2, rep=2, levels=c(-1, 0, 1)) %>% 
  fp_add_names(A="Angle", B="Speed") %>% 
  fp_add_scale(A=c(15, 25), B=c(125, 175))
...
df <- df %>% 
  left_join(
    read.table(examples_url("cutting.dat"), header=T) %>% 
...
df <- fp_design_matrix(2, rep=3) %>% 
  fp_add_names(A="Concentrazione", B="Catalizzatore")
...
```{r}
y <- read.csv(examples_url("duplicate.csv")) %>% 
...
```{r}
daniel_plot_hn(lm(t~A*B*C*D, data=df), nlab=7)
```
...
```{r}
lm(v~A*B*C*D, data=df) %>% 
  daniel_plot_hn(nlab=5)
```
...
```{r}
lm(Y~A*B*C*D, data=dm) %>% 
  daniel_plot_hn(nlab=7)
```
...
```{r}
dm <- fp_design_matrix(5) %>% 
  fp_fraction(~A*B*C*D*E) %>% 
  mutate(
    Y = c(
      9, 34, 16, 60,
      6, 50, 21, 44,  
...
```{r}
lm(Y~A*B*C*D*E, data=dm) %>% 
  daniel_plot_hn(nlab=10)
```
